function readURL(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
            jQuery('#dab_image').attr('src', e.target.result);
        };
...
jQuery(document).ready(function($) {
    ...
    $('#add-row').on('click', function() {
        ...
        jQuery('.row-' + new_row+' .rwo_field').val('row-'+Math.floor(Math.random()*(20000-1200+1)+1200));
    ...
    $('.dab-remove-row').on('click', function() {
        ...
        if(new_val ==0){
            jQuery('#repeatable-fieldset-one tbody').prepend('	<tr data-row="" class="row-1" data-tr="1">		<td>		<p class="dab-questions">Question:</p>		<input type="text" class="dabquestionnaire" name="fields[1][field_name]" />		<select name="fields[1][field_type]" data-id="1" data-tr="row-1" class="select_type"> <option value="text">Text </option>			<option value="select">Select </option>			<option value="radio">Radio </option>			<option value="checkbox">Checkbox </option>			<option value="textarea">Textarea </option>		</select>		<input type="hidden" class="rwo_field"   name="fields[1][field_row]"value="row-1" />		<div class="dab_field"></div></td>	</tr>');
        }
...
jQuery(document).on('change keyup', '#dab_state, #dabzip_code, #dab_address', function(){
    ...
    var state = jQuery('#dab_state option:selected').attr('data');
...
jQuery(document).ready( function(){
    ...
    var state = jQuery('#dab_state option:selected').attr('data');var zip =jQuery('#dabzip_code').val();
    jQuery('#searchdab').css('display','block');
    var data = {
        action:'get_office',
        'zip':zip,
